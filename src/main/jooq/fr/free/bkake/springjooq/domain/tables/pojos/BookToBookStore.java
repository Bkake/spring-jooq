/**
 * This class is generated by jOOQ
 */
package fr.free.bkake.springjooq.domain.tables.pojos;


import java.io.Serializable;

import javax.annotation.Generated;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.8.6"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class BookToBookStore implements Serializable {

    private static final long serialVersionUID = 1478766012;

    private String  name;
    private Integer bookId;
    private Integer stock;

    public BookToBookStore() {}

    public BookToBookStore(BookToBookStore value) {
        this.name = value.name;
        this.bookId = value.bookId;
        this.stock = value.stock;
    }

    public BookToBookStore(
        String  name,
        Integer bookId,
        Integer stock
    ) {
        this.name = name;
        this.bookId = bookId;
        this.stock = stock;
    }

    public String getName() {
        return this.name;
    }

    public BookToBookStore setName(String name) {
        this.name = name;
        return this;
    }

    public Integer getBookId() {
        return this.bookId;
    }

    public BookToBookStore setBookId(Integer bookId) {
        this.bookId = bookId;
        return this;
    }

    public Integer getStock() {
        return this.stock;
    }

    public BookToBookStore setStock(Integer stock) {
        this.stock = stock;
        return this;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("BookToBookStore (");

        sb.append(name);
        sb.append(", ").append(bookId);
        sb.append(", ").append(stock);

        sb.append(")");
        return sb.toString();
    }
}
